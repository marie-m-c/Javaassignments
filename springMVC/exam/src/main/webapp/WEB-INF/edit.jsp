  ...
  <body style="font-family: 'Roboto';background-image: linear-gradient(120deg, #e1e6eb 0%, #c4c9d1 50%, #a8afb5 100%);">
   <div class="container w-50 border p-4 mx-auto" style="background-color: rgba(255, 255, 255, 0.2); min-height: 100vh;">
   	<div class="d-flex align-items-center justify-content-between py-4">
   		<h2 class="fw-bold text-warning"><c:out value="${course.getName()}" /></h2>
   	</div>
    <form:form action="/classes/update/${course.getId()}" method="post" modelAttribute="course" class="form border py-4 px-4">
    	...
    	<div class="d-flex align-items-center p-1 m-1">
        	...
        	<form:input type="time" class="form-control" path="time" value="${course.getTime() }"/>
    	</div>
	...
	<c:if test="${course.getUser().getId() == userId }">
    	<form action="/classes/${course.getId()}/delete" method="post" class="my-2">
